...
class NCDMInstance(object):
		...
		if success:
			...
			if len(session.tag) > 0 and session.tag in "CX":
				...
				if pid == 0:
					...
					os.setsid()
					if session.tag == 'C':
						self.cli_session(username,next_console,session.command,fb,img,auth)
						#kill the daemon process that launched these in here
					elif session.tag == 'X':
						self.gui_session(username,next_console,session.command,ck,auth)
						#kill the daemon process that launched these in here
					else:
						os._exit(1)
						#invalid tag
	...
	def gui_session(self,username,tty,cmd,ck,auth):
	...
	def cli_session(self,username,tty,cmd,fb,img,auth):
		ttytxt='tty{}'.format(tty)
		if self.settings.logme:
			self.settings.log.info("Preparing {}'s default environment".format(username))
		usr,env=utils.make_child_env(username)
		...
		auth.setcred(PAM.PAM_ESTABLISH_CRED)
		if self.settings.logme:
			self.settings.log.debug("Registering session "
